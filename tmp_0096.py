...
def main() -> None:
    ...
    p: np.ndarray = points[0]
    m: int = n**6
    hue: float
    for i in range(m):
        p = r * p + (1-r)*points[rng.integers(0, n)]
        hue = offset + 0.5 + (i-m/2+0.5)/m*0.25
        ax.plot(p[0], p[1], linestyle="", marker="o", color=hsv2rgb(hue, saturation, value))
    # arrange
    #ax.set_xlim(0, 1)
    #ax.set_ylim(0, 1)
    ax.axis("off")
