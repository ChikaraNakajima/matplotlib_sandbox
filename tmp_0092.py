...
def main() -> None:
    ...
    for i in range(1, 1+n):
        ...
        r: np.ndarray = 1 + i/n * np.cos(6*t+i/n*np.pi)
        x: np.ndarray = r * np.cos(t)
        y: np.ndarray = r * np.sin(t)
        hue = offset + 0.25 + (i-n/2+0.5)/n*0.25
        ax.plot(x, y, linewidth=6, color=hsv2rgb(hue, saturation, value))
        hue = offset + 0.75 + (i-n/2+0.5)/n*0.25
        ax.plot(x+4, y, linewidth=6, color=hsv2rgb(hue, saturation, value))
    # arrange
    #ax.set_xlim(0, 1)
    #ax.set_ylim(0, 1)
    ax.axis("off")
